...
public class PilotXpOutcome : MissionOutcome
{
	...
	{
		PilotsManager.AwardXp(mission.pilot.id, Mathf.FloorToInt(Random.Range(xpMin, xpMax) * ApplyOmens(mission)));
	}
    ...
    private float ApplyOmens(Mission mission)
    {
        ...
        for (int i = 0; i < mission.outcomes.Length; i++)
        {
            if (mission.outcomes[i].GetType().Equals(typeof(OmenOutcome)))
            {
                ...
                coefficient += probability < Random.Range(0, 100) / 100 ? omen.coefficient : omen.coefficient * -1;
            }
        }
        return coefficient;
    }
