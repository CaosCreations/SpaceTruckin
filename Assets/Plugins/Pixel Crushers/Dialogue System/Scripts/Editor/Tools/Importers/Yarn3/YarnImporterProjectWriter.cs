...
namespace PixelCrushers.DialogueSystem.Yarn3
{
    ...
    public class YarnImporterProjectWriter
    {
        ...
        public static class EntryTitle
        {
            ...
            public const string SmartVariable = "Smart Variable";
            public const string Group = "Line Group";
            public const string GroupEnd = "Line Group End";
        ...
        public static class EntryDescription
        {
            ...
            public const string SmartVariable = "Smart Variable: name: {0} (value determined at runtime)";
...
";
        ...
        private bool _hasLoggedInfoAboutOnceConversation = false;
        private List<string> _smartVariableNames = new List<string>();
        ...
        public void Write(Yarn3ImporterPrefs prefs, YarnImporterProject project, DialogueDatabase dialogueDb)
        {
            _prefs = prefs;
        ...
        private void CreateLuaGlobalVariable(SetStatement setStmt)
        {
            ...
            // don't smash variables or create dupes. But if SmartExpression is set, assign it and add to list of smart variables.
            var dsVariable = _dialogueDb.GetVariable(setStmt.Variable);
            if (dsVariable != null)
            {
                if (setStmt is DeclareStatement declareStmt)
                {
                    if (!string.IsNullOrEmpty(declareStmt.SmartExpression))
                    {
                        dsVariable.Type = FieldType.Text;
                        dsVariable.InitialValue = FixSmartExpression(declareStmt.SmartExpression);
                        dsVariable.Description = YarnImporterProjectWriter.EntryTitle.SmartVariable;
                        if (!_smartVariableNames.Contains(dsVariable.Name))
                        {
                            _smartVariableNames.Add(dsVariable.Name);
                        }
                    }
                }
                return;
        ...
        private void FixSmartVariables()
        {
            if (_smartVariableNames.Count > 0)
            {
                Debug.Log("Yarn project has smart variables. Add YarnCustomCommands or a subclass to the Dialogue Manager and tick 'Support Smart Variables'.");

                // Change all references to smart variables such as "$abc" to "eval($abc)" instead.
                // This makes them evaluate their text content as a Lua expression at the time of use.
                _smartVariableNames.ForEach(smartVariableName => FixSmartVariable(smartVariableName));
            }
        }

        private void FixSmartVariable(string smartVariableName)
        {
            // Actor fields:
            foreach (var actor in _dialogueDb.actors)
            {
                FixSmartVariableInFields(actor.fields, smartVariableName);
            }
            // Quest fields:
            foreach (var item in _dialogueDb.items)
            {
                FixSmartVariableInFields(item.fields, smartVariableName);
            }
            // Conversations:
            foreach (var conversation in _dialogueDb.conversations)
            {
                ...
                {
                    // Replace [var=variable] references:
                    var text = entry.DialogueText;
                    if (text.Contains(smartVariableName))
                    {
                        entry.DialogueText = text.Replace($"[var={smartVariableName}]", $"[lua(eval(Variable[\"{smartVariableName}\"]))]");
                    }
                    // Replace references in conditionsString and userScript:
                    if (entry.conditionsString == null) entry.conditionsString = string.Empty;
                    if (entry.userScript == null) entry.userScript = string.Empty;
                    if (entry.conditionsString.Contains(smartVariableName) ||
                        entry.userScript.Contains(smartVariableName))
                    {
                        var regex = new Regex(@"Variable\['" + smartVariableName + @"'\]");
                        var replacement = $"eval(Variable['{smartVariableName}'])";
                        entry.conditionsString = regex.Replace(entry.conditionsString, replacement);
                        entry.userScript = regex.Replace(entry.userScript, replacement);
                    }
                }
            }
        }

        private void FixSmartVariableInFields(List<Field> fields, string smartVariableName)
        {
            if (fields == null) return;
            foreach (var field in fields)
            {
                if (field == null) continue;
                if (field.value.Contains(smartVariableName))
                {
                    field.value = field.value.Replace($"[var={smartVariableName}]", $"[lua(eval(Variable[\"{smartVariableName}\"]))]");
                }
            }
        }

        private string FixSmartExpression(string smartExpression)
        {
            var variableRegex = new Regex(@"\$\w+\b");
            var result = variableRegex.Replace(smartExpression, (match) =>
            {
                var variableName = match.Value.Substring(1);
                if (_smartVariableNames.Contains(variableName))
                {
                    var regex = new Regex(@"Variable\['" + variableName + @"'\]");
                    var replacement = $"eval(Variable['{variableName}'])";
                    return regex.Replace(match.Value, replacement);
                }
                else
                {
        ...
        private void CreateDialogueEntries()
        {
            var sortedConvoNodes = _yarnProject.Nodes.Values.OrderBy(x => x.Name).ToList();
            foreach (var convoNode in sortedConvoNodes)
            {
                CreateAndAddDialogueEntries(convoNode); // Also sets _currentConversation.
                if (convoNode.Once)
                {
                    PrependConditionalOnceNode(_currentConversation);
                    if (!_hasLoggedInfoAboutOnceConversation)
                    {
                        _hasLoggedInfoAboutOnceConversation = true;
                        Debug.Log("Yarn project has conversation(s) that play only once. Ensure Dialogue Manager's 'Include SimStatus' is ticked to support this in the Dialogue System.");
                    }
                }
            }
        }

        // Puts a node between <START> and its links that checks SimStatus.
        private void PrependConditionalOnceNode(Conversation conversation)
        {
            var entry = _template.CreateDialogueEntry(_template.GetNextDialogueEntryID(conversation), conversation.id, "<<once>>");
            conversation.dialogueEntries.Add(entry);
            entry.ActorID = DefaultNpcActorId;
            entry.ConversantID = DefaultNpcActorId;
            entry.Sequence = "Continue()";
            entry.conditionsString = "Dialog[thisID].SimStatus ~= \"WasDisplayed\"";
            var startEntry = conversation.GetFirstDialogueEntry();
            entry.outgoingLinks = new List<Link>(startEntry.outgoingLinks);
            startEntry.outgoingLinks.Clear();
            startEntry.outgoingLinks.Add(new Link(conversation.id, startEntry.id, conversation.id, entry.id));
        }
        ...
        private DialogueEntry CreateAndAddDialogueEntries(YarnStatement stmt, DialogueEntry previousEntry)
        {
            ...
            switch (stmt.Type)
            {
                ...
                case StatementType.Group:
                    nextDlgEntry = CreateRandomizedGroupEntries((GroupBlock)stmt, previousEntry);
                    break;
        ...
        private DialogueEntry CreateRandomizedGroupEntries(GroupBlock groupBlock, DialogueEntry previousEntry)
        {
            // Start with a group entry that randomly chooses from its children.
            var randomizerEntry = CreateDialogueEntry(
                _currentConversation,
                YarnImporterProjectWriter.EntryTitle.Group,
                string.Empty);
            randomizerEntry.isGroup = true;
            randomizerEntry.userScript = "RandomizeNextEntry()";
            CreateLink(previousEntry, randomizerEntry);

            // Create a group entry that all of the children will link to:
            var endGroupEntry = CreateDialogueEntry(
                _currentConversation,
                YarnImporterProjectWriter.EntryTitle.GroupEnd,
                string.Empty);
            endGroupEntry.isGroup = true;

            // Add the children:
            foreach (var childStmt in groupBlock.Statements)
            {
                var child = CreateAndAddDialogueEntries(childStmt, randomizerEntry);
                CreateLink(child, endGroupEntry);
            }

            return endGroupEntry;
        }
        ...
        private DialogueEntry CreateAndAddDialogueEntry(LineStatement stmt, DialogueEntry previousEntry)
        {
            ...
            var lclString = _yarnProject.LocalizedStringTable[stmt.LineId];
            //if (lclString.DefaultText.Contains("=>"))
            //{
            //    Debug.Log($"GROUP: {lclString.DefaultText}");
            //}
        ...
        private DialogueEntry CreateAndAddDialogueEntries(SetStatement stmt, DialogueEntry previousEntry)
        {
            ...
            var isSmartVariable = stmt is DeclareStatement declareStmt && !string.IsNullOrEmpty(declareStmt.SmartExpression);
            ...
            if (isSmartVariable)
            {
                title = YarnImporterProjectWriter.EntryTitle.SmartVariable;
                desc = string.Format(YarnImporterProjectWriter.EntryDescription.SmartVariable, stmt.Variable);
            }
            ...
            while (expStack.Count > 0)
            {
                ...
                else if (token.Type.IsOperator())
                {
                    ...
                    else if (opToken.ArgCount == 2)
                    {
                        if (argStack.Count < 2)
                        {
                            Debug.Log($"YarnProjectWriter::GenerateYarnExpressionLuaScript() - opToken.ArgCount={opToken.ArgCount} but argStack.Count={argStack.Count}");
                        }
                        else
                        {
                            lhs = argStack.Pop();
