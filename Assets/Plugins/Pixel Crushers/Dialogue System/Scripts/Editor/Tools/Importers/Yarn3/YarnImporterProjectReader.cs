...
using static PixelCrushers.DialogueSystem.Yarn3.PixelCrushersYarnSpinnerParser;
...
namespace PixelCrushers.DialogueSystem.Yarn3
{
    ...
    public class YarnImporterProjectReader : PixelCrushersYarnSpinnerParserBaseListener
    {
        ...
        // C'tor & Init Methods  --------------------------------------------------------------------------------------
        public YarnImporterProject Parse(Yarn3ImporterPrefs prefs)
        {
            _prefs = prefs;
        ...
        public string GetLineId(string nodeName, int lineNumber)
        {
            var lineIdLookup = GenerateLineIdLookup(nodeName, lineNumber);
            if (_lineIdLookupTable.TryGetValue(lineIdLookup, out var result))
            {
                return result;
            }
            else
            {
                Debug.LogWarning($"YarnImporterProjectReader::GetLineId({nodeName}, {lineNumber}) doesn't have a line ID {lineIdLookup}");
                return string.Empty;
            }
        }
        ...
        public override void EnterTitle_header([NotNull] PixelCrushersYarnSpinnerParser.Title_headerContext context)
        {
        ...
        {
             if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterHeader(): {context.header_key.Text}, {context.header_value?.Text}");
        ...
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterLine_statement() _currentNode.Name={_currentNode.Name}, lineNum={context.Start.Line}");
        ...
        //public override void EnterLineOnceCondition([NotNull] LineOnceConditionContext context)
        //{
        //    if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterLineOnceCondition()");
        //}        

        //public override void ExitLineOnceCondition([NotNull] LineOnceConditionContext context)
        //{
        //    if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitLineOnceCondition()");
        //}

        public override void EnterOnce_statement([NotNull] Once_statementContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterOnce_statement()");
        }

        public override void ExitOnce_statement([NotNull] Once_statementContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitOnce_statement()");
        }

        public override void EnterOnce_primary_clause([NotNull] Once_primary_clauseContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterOnce_primary_clause()");
            _currentNode.Once = true;
        }

        public override void ExitOnce_primary_clause([NotNull] Once_primary_clauseContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitOnce_primary_clause()");
        }

        // Handle group blocks (=>) that select a line randomly from a group:
        public override void EnterLine_group_statement([NotNull] Line_group_statementContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterLine_group_statement()");
            EnterBlockStatement(new GroupBlock());
        }

        public override void ExitLine_group_statement([NotNull] Line_group_statementContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitLine_group_statement()");
            ExitBlockStatement();
        }

        public override void EnterLine_group_item([NotNull] Line_group_itemContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterLine_group_item()");
        }

        public override void ExitLine_group_item([NotNull] Line_group_itemContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitLine_group_item()");
        }
        ...
        public override void ExitExpComparison(PixelCrushersYarnSpinnerParser.ExpComparisonContext context)
        {
            ...
            for (int i = CurrentBlockStatement.Statements.Count - 1; i >= 0; i--)
            {
                var stmt = CurrentBlockStatement.Statements[i];
                if (stmt is DeclareStatement declareStmt)
                {
                    declareStmt.SmartExpression = prevExpr;
                    break;
                }
            };            
        }
        ...
        public override void EnterValueTypeMemberReference([NotNull] ValueTypeMemberReferenceContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::EnterValueTypeMemberReference()");
            var text = context.GetText();
            var token = new StringToken(text);
            //Debug.Log($"YarnProjectListener::EnterValueTypeMemberReference(): {text}");
            CreateAndPushToken(token);
        }

        public override void ExitValueTypeMemberReference([NotNull] ValueTypeMemberReferenceContext context)
        {
            if (_prefs.debug) Debug.Log($"YarnProjectListener::ExitValExitValueTypeMemberReferenceeFalse()");
        }
        ...
        public override void EnterDeclare_statement(PixelCrushersYarnSpinnerParser.Declare_statementContext context)
        {
            ...
            // Save the expression in case it's for a smart variable:
            var expression = context.expression();
            prevExpr = expression.GetText();
